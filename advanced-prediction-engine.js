class AdvancedPredictionEngine {
    ...
    predict(recentGames, currentGame, betAmount) {
        ...
        // Registrar predição para aprendizado
        this.recordPrediction(prediction, currentGame);
    ...
    analyzeAdvancedPatterns(recentGames) {
        ...
        const analysis = {
            ...
            autocorrelacao: this.calculateAutocorrelation(colors),
            ciclos: this.detectCycles(colors),
            numeroQuentes: this.getHotNumbers(rolls),
            numerosFrios: this.getColdNumbers(rolls),
            intervalosTemporais: this.analyzeTimeIntervals(timestamps),
    ...
    varianceStrategy(recentGames) {
        ...
        // Alta variância = mais imprevisível, baixa variância = mais padrão
        let prediction = 1;
        let confidence = 0.5;
    ...
    calculateSmartBettingStrategy(totalAmount, prediction, analysis) {
        ...
        if (analysis.tendencia !== 'neutro') {
            whiteAmount = Math.max(10, whiteAmount); // Mínimo de segurança
        }
    ...
    detectCycles(colors) {
        const cycles = [];
        // Detectar ciclos de 3 a 8 elementos
    ...
    calculateCorrectionFactor(analysis) {
        ...
        if (analysis.entropia < 1.0) factor *= 1.1; // Baixa entropia = mais previsível
        if (analysis.entropia > 1.8) factor *= 0.9; // Alta entropia = menos previsível
    ...
    // Ajustar pesos baseado na performance (implementação básica)
    adjustWeights(strategies) {
        // Aqui poderia implementar machine learning
        // Por agora, mantém os pesos padrão
        return this.strategyWeights;
    }

    recordPrediction(prediction, currentGame) {
        this.predictionHistory.push({
            prediction,
            actualResult: currentGame,
            timestamp: new Date()
        });
